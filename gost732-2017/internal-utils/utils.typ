...
#let should_be_unnumbered_heading(heading) = {
    ...
    let match_res = heading.match(regex(
        "^(аннотация|список исполнителей|реферат|содержание|термины и определения|определения, обозначения и сокращения|перечень сокращений и (?:обозначений|ссылок)|введение|заключение|список использованных источников|приложение [а-яё].*)$"
    ))
...
#let should_be_upper_heading(heading) = {
    ...
    let match_res = heading.match(regex(
        "^(аннотация|список исполнителей|реферат|содержание|термины и определения|определения, обозначения и сокращения|перечень сокращений и (?:обозначений|ссылок)|введение|заключение|список использованных источников)$"
    ))
