...
if all(m is not None for m in (m_alpha, m_beta, m_gamma)):
    ...
    # Зафиксированный факт поведения: вложенный пункт инлайнится в строку родителя
    # «альфа» (общий baseline). Если когда-нибудь вложенный начнёт переноситься на
    # отдельную строку — этот инвариант FAIL и про регрессию станет известно.
    c.check("nested_inline_with_parent",
            abs(m_beta[2] - m_alpha[2]) < 3,
            f"ОЖИДАЕМОЕ поведение пакета — вложенный пункт на одном baseline с родителем; "
            f"y(бета)={m_beta[2]:.1f}, y(альфа)={m_alpha[2]:.1f} разошлись "
            f"(поведение изменилось: вложенность теперь переносит строку)")
