...
# Нативный enum выравнивает номера по ПРАВОМУ краю (двузначные начинаются
# левее), за счёт чего колонка текста ровная. Проверяем правый край маркеров.
mxr = [markers[n][3] for n in range(1, 13)]
c.check("markers_right_aligned", max(mxr) - min(mxr) < 1.5,
        f"правый край маркеров не на одной вертикали: {[round(x,1) for x in mxr]}")
